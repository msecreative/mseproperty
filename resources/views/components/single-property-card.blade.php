<div class="flex-1 mx-2 shadow-lg relative rounded-md mb-6">
    ...
    <div class="py-20 bg-center" style="background-image: url('/storage/uploads/{{ $property->featured_image }}'); background-repeat: no-repeat; background-position: center; background-size: cover;"></div>
    <div class="p-3">
        <h2 class="leading-0 text-base">{{$property->name}}</h2>

        <h3 class="text-2xl py-3">{{$property->dynamicPricing($property->price)}}</h3>
        <div class="border-t-2">
            <ul class="flex items-center -mx-1 my-4">
                ...
                <li class="px-2 py-1 bg-blue-50 rounded-md text-xs mx-1 shadow-sm">{{$property->gross_sqm}} ft<sup>2</sup></li>
            </ul>
            <a target="_blank" href="{{route('single-property', $property->id)}}" class="btn w-full text-center">More details</a>
        </div>
    </div>
</div>